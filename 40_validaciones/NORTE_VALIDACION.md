...
|---|---|---|---|---|---|---|
| 2.1.2 | **Rendimiento plazo fijo** | [[K-DEV-003]] | S-DEV-002 *(por escribir)* | `oraculo_rendimientos.py::rendimiento_plazo` · `validate_plazo_origin.py` · `V5` | `aurumcore.iv_payment_plan` (`origin`, `interest_amount`, `interest_paid`) | ✅ **C=B a VOLUMEN**: oráculo reproduce el plazo generado por Aurum (`origin null`) **100% — 0 violaciones en 530,195 periodos / 157,999 cuentas**. Migrado (C=A) 97.8%. Valida fórmula/redondeo; tasa contratada vs esquema = pendiente menor |
