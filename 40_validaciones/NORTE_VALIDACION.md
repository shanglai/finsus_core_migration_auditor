...
>
> Actualizado: 2026-08-19 · Ligado a: `PLAN_DE_VALIDACION.md` (fases 0-9), `CATALOGO_VALIDACIONES.md`
...
|---|---|---|---|---|---|---|
| 2.1.2 | **Rendimiento plazo fijo** | [[K-DEV-003]] | S-DEV-002 *(por escribir)* | `oraculo_rendimientos.py::rendimiento_plazo` (autoprueba 3/3) · `V5_rendimiento_plazo.sql` | `aurumcore.iv_payment_plan.interest_amount` (JOIN `account`) | ✅ VALIDADO 775/775 · pendiente **escalar muestra** |
...
| # | motor / métrica | regla (pieza) | spec | validación / código | dato en BD · cómo ubicarlo | estado |
|---|---|---|---|---|---|---|
| 2.1.7 | **Suspensión de devengo / IDNC** (cartera vencida) | [[K-REG-001]] v2 | — | `V3_gapB_idnc.sql` | `aurumcore.lc_finantial_data_stage` (IODNC); ejemplo crédito `123-1515-1837` | 🟡 **existe + lógica DOCUMENTADA** (F-023: 90d, IO_VENC, reserva 100% `RESERVA_INT_*`, cuentas de orden). Falta **cuadre en datos** (ya no bloqueado por doc) |
...
| # | motor / métrica | regla (pieza) | spec | validación / código | dato en BD · cómo ubicarlo | estado |
|---|---|---|---|---|---|---|
| — | **Matriz de amarre `tipo_mov → cuenta`** | [[K-CTB-001]] | — | familias **B** (balanza) y **C** (amarre) *(sin construir)* | balanza / auxiliar AurumCore | 🟠 matriz documentada · invariantes contables **pendientes** (tolerancia 0.00) |
...
## 5. El VALIDADOR ya existe (2026-08-20)

Construido en `validador/`. Este NORTE sigue siendo su fuente de "qué se valida"; el
catálogo `validador/catalogo/*.yaml` es su forma ejecutable, y ambos deben mantenerse
sincronizados (misma nomenclatura y estado).

| | |
|---|---|
| Operación | `python cli.py --listar` · `--explicar <ID>` · `--caso <ID> --dry-run` · `--confirmar` |
| Cobertura real | `validador/reportes/cobertura.md` (`python cli.py --cobertura`) |
| Autopruebas | `python cli.py --autopruebas` — 173 pruebas sin BD, incluidos los casos-trampa |

**Correspondencia NORTE → caso del catálogo:**

| motor / métrica de este NORTE | caso | estado en el catálogo |
|---|---|---|
| ISR retención al pago | `ISR-01` | VALIDADO |
| ISR devengo diario (motor A) | `ISR-02` | VALIDADO |
| Parámetros ISR vs norma | `ISR-03` | VALIDADO · **caso-trampa vivo de C-001** |
| Rendimiento plazo fijo (2.1.2) | `REND-PLAZO` | VALIDADO · consulta ya escalada a cohorte |
| Interés vista/ahorro (2.1.1) | `REND-VISTA` | PARCIAL — falta extracción (P-015) |
| Saldo promedio (2.1.3) | `SALDO-PROM` | BLOQUEADO — P-006 |
| Transaccional diaria | `DIARIO-B` | PARCIAL — P-016 + catálogo 2:1/1:1 de K-MOV-001 |
| Suspensión devengo / IDNC (2.1.7) | `GAPB-IDNC` | PARCIAL — falta spec de reserva IFRS9 |
| Cuota Prosofipo | `GAPC-PROSOFIPO` | HALLAZGO — invariante de regresión |
| Write-offs | `WRITEOFFS` | PENDIENTE — falta pieza K |
| Devengamiento One Click | `CRED-IO` | PENDIENTE — **falta la regla** (P-006) |
| Familias contables B/C | `CONTABLE-BC` | PENDIENTE — el hueco más grande en severidad 1 |
| "¿Se come todas?" | `COMPLETITUD` | PARCIAL — **único ejecutable de punta a punta hoy** |

> **Ojo con `estado`.** Describe lo que se sabe de la *validación* (según este NORTE), no si
> el validador la corrió. Eso último vive en `cobertura.md`: los casos marcados VALIDADO lo
> fueron con los comparadores anteriores, y hasta que el validador los corra aparecen
> **NO-CORRIDO**. No son sinónimos de "pasa".
