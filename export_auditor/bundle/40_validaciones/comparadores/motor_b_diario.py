...
def main():
    ...
    # METODOLOGIA: se filtra `transaction.origin IS NULL` = generado por AurumCore (live), excluyendo lo
    # migrado/ingestado (valores con nombre: FINSUS_INVESTMENT, 841, FINSUS_YIELD...). Asi comparamos SOLO
    # lo que AurumCore calculo, no lo ingestado (K-MIG-002, P-013). En consonancia con los queries de Sergio
    # (F-027), que filtran `origin is null`. `origin` vive en `transaction`, no en `transaction_detail` -> join.
    au_tipos = rows(cau, """
        select td.transaction_type, count(*) n
        from aurumcore.transaction_detail td
        join aurumcore.transaction t on t.transaction_id = td.transaction_id
        where td.created::date=%s and t.origin is null
        group by 1 order by n desc""", (fecha,))
