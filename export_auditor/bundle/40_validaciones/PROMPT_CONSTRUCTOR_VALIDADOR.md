...
> **Fuente única (decisión C):** el catálogo de casos vive en **`NORTE_VALIDACION.md`** (matriz maestra §0–§1).
> Este repo NO mantiene un catálogo paralelo: el `catalogo/*.yaml` del auditor es un **espejo** del NORTE. La
> tabla de siembra de abajo es un snapshot; la verdad vigente (estado, cobertura) es el NORTE. Sincronización:
> `export_auditor/PROMPT_SYNC_AUDITOR.md`.

Cada caso es un archivo declarativo en `catalogo/` (espejo de la fila del NORTE). **Plantilla obligatoria**
(`_schema.md`):
