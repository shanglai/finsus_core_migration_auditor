...
def construir(motor, autopruebas: dict, con_bd: bool, params: dict) -> dict:
    d = motor.como_dict()
...
def main(argv=None) -> int:
    ...
    for motor in seleccion:
        d = construir(motor, autopruebas, args.con_bd, params)
        ...
        if d["cruce"] is None and previo.exists():
            try:
                anterior = json.loads(previo.read_text(encoding="utf-8"))
                if (anterior.get("cruce") or {}).get("origen_resultado") == "corrida_local":
                    d["cruce"] = anterior["cruce"]
                    d["origen_resultado"] = "corrida_local"
                    d["pct_mostrado"] = anterior["cruce"]["pct_match"]
